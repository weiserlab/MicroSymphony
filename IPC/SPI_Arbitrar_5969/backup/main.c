...
static void uart_send_u8(uint8_t b)
{
...
static void arbiter_schedule(void)
{
    ...
    P4OUT |= BIT6;
    uart_print_str("Granted bus to node ");
    uart_send_u8((uint8_t)('0' + next));
    uart_print_str("\r\n");
}
...
int main(void)
{
    ...
    uart0_init();
    arbiter_gpio_init();
    if (fram_init()) uart_print_str("FRAM initialized\r\n");
    else { uart_print_str("FRAM init failed\r\n"); P4OUT |= BIT6; P1OUT |= BIT0; return 0; }
    ...
    for (;;) {
        ...
        // arbiter_check_notifications(); /** */ Disabled for now */
